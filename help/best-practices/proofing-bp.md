...
* Workfront設定の「プルーフにコメントが作成された場合にWorkfrontからメールを送信」設定を無効にします。
...
デフォルトは 24 時間形式なので、慣れていない人にとっては混乱する可能性があります。 フォーマットを変更するには、Workfrontのメインメニュー/プルーフ/アカウント設定/ユーザーに移動します。 ユーザーをダブルクリックして選択し、「個人設定」セクションの「日付形式」フィールドを編集します。 ユーザーを 1 つずつ選択して変更する必要があります。

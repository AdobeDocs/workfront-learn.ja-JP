...
「現在」ステータスの実際の作業プロジェクトとリクエストを区別するには、「リクエストキュー」または「運用中」というリクエストキューでのみ使用するステータスを作成します。 次に、このステータスを使用して、レポートの作成中にリクエストキュープロジェクトを除外または含めることができます。
...
ほとんどの場合、リクエストキューは、チーム、ベンダー、顧客など、特定の人のセットと共有する必要があります。リクエストキューのリストで必要な情報のみを表示することで、容易に情報を見つけて移動できます。

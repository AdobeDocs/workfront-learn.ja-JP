---
title: タスクレポートの作成
description: 複雑なフィルターを使用してタスクレポートを作成し、Workfrontで作成したレポートを見つける方法について説明します。
activity: use
...
ビデオでは、ログインしたユーザーに割り当てられた遅延タスクを追跡するカスタムレポートを作成する手順を順を追って説明します。 既存のフィルター、ビュー、ビルトインレポートが特定の要件を満たしていない場合は、まずカスタムレポートの必要性を説明します。&#x200B; レポートの名前は「自分に割り当てられた遅延タスク」で、説明がわかりやすいように追加されています。&#x200B;
...
* **カスタムレポートの作成：** 既存のフィルターや組み込みレポートがニーズを満たさない場合は、カスタムレポートをゼロから作成して、割り当てられた遅延タスクなど、特定のデータに焦点を当てることができます。&#x200B;
* **フィルター設定：** フィルターを使用して、遅延タスク、未完了タスク、現在のプロジェクト、ログインしているユーザーに割り当てられたタスクなどのレポート条件を定義します。&#x200B;
* **コンテキストの列：** 依存関係の問題を特定するための「開始可能」や、関連プロジェクトに簡単に移動できるハイパーリンクを含んだ「プロジェクト名」などの列を追加します。&#x200B;
* **優先度による並べ替え：** 最も期限切れのタスクをレポートの先頭に優先付けするために、「期限」列で昇順にタスクを並べ替えます。&#x200B;
* **アクセシビリティ：** レポートを保存、ピン留め、またはお気に入りとしてマークしてすばやくアクセスできるので、遅延タスクを効率的に追跡および管理できます。
...
このカスタムフォームは、プロジェクトタイプとイシューオブジェクトタイプを使用し、次のように作成する必要があります。

名前：管理プロセスのフィードバック

1. プロセスタイプ（単一選択ドロップダウンフィールド）
   * アクセスレベル
   * 承認プロセス（グローバルのみ）
   * メール通知
   * レイアウトテンプレート

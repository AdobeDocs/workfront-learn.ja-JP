...
* **ビューと監視：**&#x200B;計画の標準ビューや監視のステータスビューなど、Workfrontのさまざまなビューには、進行状況、コメント、ドキュメント、イシュー、承認、クリティカルパス、マイルストーンのフラグが含まれます。最近のアクティビティも追跡できます&#x200B;
* **スケジュール設定オプション：** プロジェクトは、開始日または完了日からスケジュールできます。Workfrontでは、タスクの期間と先行タスクに基づいて対応する日付を計算します。&#x200B;重要完了日には、開始日からのスケジュール設定を推奨し、一部の余裕を許可&#x200B;ます。
* **タスクの制約：** 「できるだけ早く」や「できるだけ遅く」などのタスクの制約は、タスクのスケジュール設定に影響します。&#x200B; プロジェクト作成後にスケジュールモードを変更すると、タスクの制約や予定日に影響を与える可能性があります。&#x200B; タスクの制約を表示するために、カスタムビューを作成できます。&#x200B;
